...
const mostrarProductosDelCarrito = async () => {
  ...
  try {
      ...
      productos += `
        <div class="productos-carrito">
          ...
          <div class="div-botones">
            <button onclick="eliminarProductoDeCarrito(${articulo.id})" class="btn btn-outline-dark">Eliminar</button>
    ...
    resumenDeCmpra += `
        <span>Total: $ ${acumulador.toFixed(2)}</span>
...
const eliminarProductoDeCarrito = async (id_producto) => {
  const token = localStorage.getItem("token");
  const id_usuario = localStorage.getItem("userid");
  if (!token || !id_usuario) {
    alert("Necesitas iniciar sesión para eliminar del carrito.");
    window.location.href = "../HTMLS/login.html";
    return;
  }
  try {
    const response = await fetch("http://localhost:3000/api/carrito/delete", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
    ...
    console.error("Error al elimnar del carrito:", error.message);
    alert("Hubo un error. Intenta de nuevo.");
  }
};
